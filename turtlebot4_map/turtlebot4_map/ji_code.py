...
class RobotVacuumMapper(Node):
    ...
    def map_callback(self, msg): #map에 대한 정보를 가져와
        ...
        self.width = msg.info.width #가로 길이
    ...
    def odom_callback(self, msg): #내 로봇 위치 가져오기
        ...
        grid_x = int((self.robot_x - self.origin.position.x) / self.resolution) #현재 로봇 float길이를 좌표로 변환
        grid_y = int((self.robot_y - self.origin.position.y) / self.resolution) #현재 로봇 float길이를 좌표로 변환
        ...
        if self.grid is not None: #만약 map에 대한 좌표 정보 리스트있다면
            ...
            self.check_goal_reached(closest_unknown[0],closest_unknown[1]) #가장 가까운 좌표로 send_goal
    ...
    def check_goal_reached(self,target_x, target_y): #send_goal이 True이면 send_goal을 보내는 과정으로 이동 아니라면 안보냄
        if self.goal_sent: #만약 goal보내기 승인이 되었다면
            print(f'target_x={target_x}') #closest_unknown[0]
            print(f'target_y={target_y}') #closest_unknown[1]
    ...
    def follow_path(self, target_x, target_y): #받은 좌표로 이동하기
        ...
        while not goal_reached: #도착을 못했다면
            ...
            self.send_nav_goal(trans_target_x, trans_target_y) #send_goal
            print('여기있어요')
            ...
            while not feedback_received: #피드백이 없다면
                ...
                if self.robot_x == target_x and self.robot_y == target_y:  # 만약 현재 제공 받은 로봇 좌표와 target좌표를 비교하여 도착여부를 확인
